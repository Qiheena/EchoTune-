...
const generateCommandInfoEmbed = (clientCmd, interaction) => {
  ...
  const {
    data, cooldown, clientPerms, userPerms, category, aliases
  } = clientCmd;
  ...
  // Build usage examples based on command options
  let usageExample = `${prefix}${data.name}`;
  if (data.options && data.options.length > 0) {
    const firstOption = data.options[0];
    if (firstOption.type === 3) { // String type
      usageExample += ` <${firstOption.name}>`;
    } else if (firstOption.type === 4 || firstOption.type === 10) { // Integer/Number type
      usageExample += ` <${firstOption.name}>`;
    }
  }
  ...
  if (aliases && aliases.length > 0) {
    aliasText = `\n**Shortcuts:** ${aliases.map(a => `\`${prefix}${a}\``).join(', ')}`;
  }

  const fields = [
    {
      name: `${emojis.info} How to use`,
      value: `\`${usageExample}\`${aliasText}`,
      inline: false
    },
    {
      name: 'Category',
      value: titleCase(category.replace(/-/g, ' ')),
      inline: true
    },
    {
      ...
      }** per **${cooldown.duration}s**`,
      inline: true
    }
  ];
  
  // Only show permissions if they're required
  if (clientPerms.length > 0 || userPerms.length > 0) {
    fields.push({
      name: 'Required Permissions',
      value: `**Bot:** ${clientPerms.length > 0 ? resolvePermissionArray(clientPerms).join(', ') : 'None'}\n**User:** ${userPerms.length > 0 ? resolvePermissionArray(userPerms).join(', ') : 'None'}`,
      inline: false
    });
  }
  ...
  return {
    ...
    title: `${emojis.success} ${titleCase(data.name)} Command`,
    description: `${data.description}`,
    fields,
    footer: { text: `Prefix commands only • Use ${prefix}help for all commands` }
  };
};
...
const generateCommandOverviewEmbed = (commands, interaction) => {
  ...
  // Filter commands user has access to
  const availableCommands = commands
    .concat(contextMenus)
    .filter((cmd) => cmd.permLevel <= member.permLevel && cmd.enabled === true);

  // Organize commands by category with better grouping
  const categoryMap = {
    'Music': [],
    'Music DJ': [],
    'Music Admin': [],
  ...
  availableCommands.forEach((cmd) => {
    ...
    if (cat === 'Music Dj') {
      categoryMap['Music DJ'].push(cmd);
    } else if (cat === 'Music Admin') {
      categoryMap['Music Admin'].push(cmd);
    } else if (categoryMap[cat]) {
      categoryMap[cat].push(cmd);
    }
  });

  // Build fields with prefix and better formatting
  const fields = [];
  
  // Add description at top
  fields.push({
    name: `${emojis.info} How to use commands`,
    value: `All commands start with **\`${prefix}\`**\nExample: **\`${prefix}play Faded\`** or **\`${prefix}p Faded\`**\n\u200B`,
    inline: false
  });

  // Add each category
  Object.entries(categoryMap).forEach(([category, cmds]) => {
    if (cmds.length > 0) {
      ...
      fields.push({
        name: `${emojis.separator} ${category} Commands`,
        value: commandList,
        inline: false
      });
    }
  ...
  // Add helpful footer
  fields.push({
    name: `${emojis.wait} Need more help?`,
    value: `Use **\`${prefix}help <command>\`** for detailed info\nExample: **\`${prefix}help play\`**`,
    inline: false
  });

  return {
    title: `${emojis.success} Music Bot Commands`,
    description: `Server: **${guild.name}**\nPrefix: **\`${prefix}\`**`,
    color: colorResolver(colors.main),
    fields,
    footer: { text: `Total commands: ${availableCommands.size} • Requested by ${member.user.username}` }
  };
};
