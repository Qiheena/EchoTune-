...
module.exports = (client, interaction) => {
  ...
  // Block only ApplicationCommand (slash commands), allow buttons/components
  ...
  // Allow button/component interactions (MessageComponent type)
  // Check if this is a valid interaction
  if (!checkInteractionAvailability(interaction)) return;
  
  // Setting permission level for this member
  const permLevel = getPermissionLevel(clientConfig, interaction.member, interaction.channel);
  interaction.member.permLevel = permLevel;
  
  // Get activeId
  const activeId = interaction.customId || interaction.commandName;
